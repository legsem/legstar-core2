...
/**
 * Exposes the {@link Cob2ConverterGenerator} utility as a command line tool.
 *
 */
public class Cob2ConverterGeneratorMain {
    ...
    private static final String OPTION_OUTPUT_PACKAGE = "package";
    ...
    private static Logger log = LoggerFactory
            .getLogger(Cob2ConverterGeneratorMain.class);
    ...
    public static void main(final String[] args) {
        Cob2ConverterGeneratorMain main = new Cob2ConverterGeneratorMain();
        main.execute(args);
    }
    ...
    public void execute(final String[] args) {
        ...
        try {
            ...
            if (collectOptions(options, args)) {
                setDefaults();
                Cob2ConverterGenerator gen = new Cob2ConverterGenerator(configProps);
                if (input.isDirectory()) {
                    for( File cobolFile : FileUtils.listFiles(input, null, true)) {
                        generate(gen, cobolFile, inputEncoding, outputPackage);
                    }
                } else {
                    generate(gen, input, inputEncoding, outputPackage);
                }
            }
        } catch (Exception e) {
    ...
    private void generate(Cob2ConverterGenerator gen, File cobolFile,
        ...
        try {
            ...
            for (Entry < String, String > entry : code.entrySet()) {
                ...
                log.info("Writing java class " + className + " with package " + packageName + " in " + output);
                FileUtils.writeStringToFile(
                        new File(output, subFolder + className),
                        entry.getValue());
            }
        } catch (IOException e) {
    ...
    protected Options createOptions() {
        ...
        Option outputPackage = new Option("p", OPTION_OUTPUT_PACKAGE, true,
    ...
    private boolean processLine(final CommandLine line, final Options options) {
        ...
        if (line.hasOption(OPTION_OUTPUT_PACKAGE)) {
            setOutputPackage(line.getOptionValue(OPTION_OUTPUT_PACKAGE).trim());
        }
    ...
    /**
     * Check that package name provided is valid.
     * <p/>
     * Null is acceptable and interpreted as no package.
     * @param outputPackage the generated java classes package name
     */
    public void setOutputPackage(String outputPackage) {
        if (outputPackage == null || outputPackage.length() == 0) {
            this.outputPackage = null;
            return;
        }
        Matcher matcher = PACKAGE_NAME_PATTERN.matcher(outputPackage);
        if (matcher.matches()) {
            this.outputPackage = outputPackage;
        } else {
            throw (new IllegalArgumentException(
                    "The name " + outputPackage + " is not a valid java package name"));
        }
