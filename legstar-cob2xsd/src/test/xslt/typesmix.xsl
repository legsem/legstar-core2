...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xsd="http://www.w3.org/2001/XMLSchema" xmlns:cb="http://www.legsem.com/legstar/xml/cobol-binding-1.0.1.xsd" xmlns:jaxb="http://java.sun.com/xml/ns/jaxb">
    ...
    <!-- Change XSD type from xsd:string to xsd:hexBinary -->
    <xsl:template match="*/xsd:restriction[../..//cb:cobolElement/@cobolName='C-OCTET-STRING']">
        <xsl:copy>
            <xsl:copy-of select="@*"/>
    ...
    <xsl:template match="*/cb:cobolElement[@cobolName='C-OCTET-STRING']">
        <xsl:copy>
            <xsl:copy-of select="@*"/>
